    ...
    <div className="dashboard">
      ...
      <div className="dash-greeting">
        ...
        <div className="dash-greeting-hero">
          <div className="dash-greeting-hero-stat">
            <span className="dash-greeting-hero-label">Portfolio Value</span>
            <span className="dash-greeting-hero-value">
              <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="currentColor" width="28" height="28" style={{ flexShrink: 0, opacity: 0.7, display: 'block' }}>
            ...
            {pricesLoaded && (
              <span className="dash-greeting-hero-sub">
                <img src="https://csfloat.com/favicon.ico" alt="CSFloat" width="18" height="18" style={{ borderRadius: 3, flexShrink: 0 }} />
                ${totalCsfloat.toFixed(2)}
              </span>
            )}
          </div>
          <div
            className={`dash-greeting-hero-stat${change24h != null ? ' dash-greeting-hero-stat--clickable' : ''}`}
            onClick={change24h != null ? () => setShowMovers(true) : undefined}
            title={change24h != null ? 'Click to see top movers' : undefined}
          >
            <span className="dash-greeting-hero-label">Daily Change</span>
            <span className="dash-greeting-hero-value">
              {change24h == null
                ? <span className="dash-stat-loading">—</span>
                : <span className={change24h >= 0 ? 'perf-up' : 'perf-down'}>
                    {change24h >= 0 ? '+' : '-'}${Math.abs(change24h).toFixed(2)}
                  </span>
              }
            </span>
            {change24h != null && (
              <span className={`dash-greeting-hero-pct item-price-pct ${change24h >= 0 ? 'pct-up' : 'pct-down'}`}>
                {change24hPct >= 0 ? '+' : '-'}{Math.abs(change24hPct).toFixed(1)}%
              </span>
            )}
          </div>
        </div>
